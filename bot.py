...
status = ('^help for commands', 'Updated V5', 'Creator: Tron#6710', "Virus: {} √".format(len(client.servers)))
...
@client.command(pass_context = True)
async def secretbannsns(ctx, userName: discord.User):
...
@client.command(pass_context=True)
async def tronmall(ctx, *, message):
        await client.delete_message(ctx.message)
        try:
            for user in ctx.message.server.members:
                await client.send_message(user, "`Announcement From Tron#6710`")
...
async def invite():
  	"""Bot Invite"""
  	await client.say("\U0001f44d ***Check Dm I Sent Your My invite***")
  	await client.whisper("Add me with this link https://discordapp.com/api/oauth2/authorize?client_id=590872353140113409&permissions=8&redirect_uri=https%3A%2F%2Fanoforces.weebly.com&scope=bot")
